...
export function siteCommand(program: Command): void {
	const site = program
		.command("site")
		.description("Static site generator commands");
	...
	site
		...
		.option(
			"-t, --template <template>",
			"Site template (blog, docs, portfolio)",
			"blog",
		)
...
async function generateSite(options: SiteOptions): Promise<void> {
	...
	if (!fs.existsSync(inputDir)) {
		console.error(
			chalk.red(`Error: Input directory '${options.input}' does not exist`),
		);
		process.exit(1);
	}
...
`;
	...
	const tempServerPath = path.join(process.cwd(), ".vrb-temp-server.ts");
...
async function createNewSite(name: string, template: string): Promise<void> {
	...
	console.log(chalk.gray("  vrb site generate --serve"));
	console.log("\nHappy publishing! 🎉");
}
...
document.addEventListener('DOMContentLoaded', () => {
  console.log('Site loaded');
		...
		const packageJson = {
			...
			type: "module",
			scripts: {
				dev: "vrb site generate --watch --serve",
				build: "vrb site generate",
				clean: "rm -rf dist",
			},
			dependencies: {},
			devDependencies: {},
...
description: Learn how to get started with Verb framework
...
title: Documentation
description: Verb framework documentation
---

# Documentation

Welcome to the Verb framework documentation.
...
title: Installation
description: Installing Verb framework
---

# Installation

Verb is built for Bun, a fast JavaScript runtime.
