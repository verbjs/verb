...
/**
 * Remote address information for UDP messages
 */
...
export class UDPClient {
  private socket: ReturnType<typeof udpSocket>;
  ...
  sendAndReceive(
    ...
    address: string,
    timeout = 5000,
  ): Promise<{ message: Buffer; rinfo: RemoteInfo }> {
    return new Promise((resolve, reject) => {
  ...
  return createUDPServer({
    ...
    handlers: {
      message(message, rinfo) {
        ...
        const socket = this as ReturnType<typeof createUDPServer>;
