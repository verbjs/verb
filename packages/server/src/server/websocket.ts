...
export const createWebSocketServer = (options: WebSocketServerOptions = {}) => {
	...
	const server = serve({
		...
		websocket: {
			...
			open(ws) {
				console.log(
					`WebSocket connection opened at ${new Date().toISOString()}`,
				);
				websocket.open?.(ws);
			},
	...
	return {
		...
		subscribe: (ws: ServerWebSocket<WebSocketData>, topic: string) => {
			ws.subscribe(topic);
		},
		/** Unsubscribe a connection from a topic */
		unsubscribe: (ws: ServerWebSocket<WebSocketData>, topic: string) => {
	...
	connect(): Promise<void> {
		return new Promise((resolve, reject) => {
			try {
				...
				this.ws.onopen = () => {
					this.handlers.open?.(this.ws as ServerWebSocket<WebSocketData>);
					resolve();
				};
				...
				this.ws.onerror = (event) => {
					...
					type ErrorEvent = { error?: Error };
					const err =
						(event as ErrorEvent).error instanceof Error
							? (event as ErrorEvent).error
							: new Error("WebSocket error event");
					this.handlers.error?.(this.ws as ServerWebSocket<WebSocketData>, err);
					reject(err);
				};
			} catch (error) {
				reject(error);
...
export const createChatServer = (port = 3001) => {
	const _rooms = new Map<string, Set<ServerWebSocket<WebSocketData>>>();
