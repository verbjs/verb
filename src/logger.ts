 ...
 */
const createFileDestinationState = (
	filePath: string,
): FileDestinationState => ({
	filePath,
	writer: undefined,
});
 ...
 */
const fileDestinationFlush = async (
	state: FileDestinationState,
): Promise<void> => {
 ...
 */
const createConsoleDestinationState = (
	prettyPrint = false,
): ConsoleDestinationState => ({
	prettyPrint,
});
 ...
 */
const consoleDestinationWrite = (
	state: ConsoleDestinationState,
	entry: LogEntry,
): void => {
...
export const log = async (
	...
	if (!shouldLog(state, level)) return;
...
export const logRequest = async (
	...
	if (!shouldLog(state, LogLevel.INFO)) return;
