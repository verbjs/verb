...
export const createMemorySessionStore = (): SessionStore => {
	...
	return {
		...
		set: (sessionId: string, session: Session) =>
			memoryStoreSet(state, sessionId, session),
...
export const createRedisSessionStore = (
	...
	return {
		...
		set: (sessionId: string, session: Session) =>
			redisStoreSet(state, sessionId, session),
 ...
 */
export const createSessionManagerState = (
...
export const createSessionMiddleware = (state: SessionManagerState) => {
	return async (
		...
		let sessionId = cookies[state.options.cookie.name!];
		...
		if (!session) {
			...
			const now = new Date();
			const expiresAt = new Date(now.getTime() + state.options.cookie.maxAge!);
		...
		if (shouldSave) {
			if (state.options.rolling) {
				session.expiresAt = new Date(Date.now() + state.options.cookie.maxAge!);
			}
			session.updatedAt = new Date();
		...
		const cookie = serializeCookie(state.options.cookie.name!, cookieValue, {
