...
const createPluginContext = (
	...
	addRoute: (method: string, path: string, handler: Handler) => {
		if (!server) {
			console.warn(
				`[${plugin.name}] Cannot add route - no server context available`,
			);
			return;
		}
		...
		if (typeof server[methodLower] === "function") {
			...
			console.warn(
				`[${plugin.name}] Server method ${methodLower} not available`,
			);
		}
	},
	addMiddleware: (middleware: Middleware) => {
		if (!server) {
			console.warn(
				`[${plugin.name}] Cannot add middleware - no server context available`,
			);
			return;
		}
	...
	if (metadata.dependencies) {
		for (const dep of metadata.dependencies) {
			if (!state.plugins.has(dep)) {
				throw new Error(
					`Plugin '${metadata.name}' depends on '${dep}' which is not registered`,
				);
			}
		}
	}
 ...
 */
export const startPlugins = async (
	state: PluginManagerState,
): Promise<PluginManagerState> => {
	if (state.isStarted) {
...
export class PluginManager {
	...
	updateServer(server: any): void {
		...
		for (const [pluginName, context] of this.state.pluginContexts) {
