...
const processPart = async (
  ...
  if (headers.filename !== undefined) {
    if (content.length > options.maxFileSize) {
      throw new Error(`File too large: ${content.length} bytes (max: ${options.maxFileSize})`);
    }
    ...
    if (content.length > options.maxFieldSize) {
      throw new Error(`Field too large: ${content.length} bytes (max: ${options.maxFieldSize})`);
    }
