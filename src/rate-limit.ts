 ...
 */
const createMemoryStoreState = (cleanupIntervalMs = 60000): MemoryStoreState => {
 ...
 */
const memoryStoreGet = (state: MemoryStoreState, key: string): Promise<RateLimitInfo | null> => {
 ...
 */
const memoryStoreReset = (state: MemoryStoreState, key: string): Promise<void> => {
 ...
 * Create memory store
 */
export const createMemoryStore = (cleanupIntervalMs = 60000): RateLimitStore => {
  const state = createMemoryStoreState(cleanupIntervalMs);

  return {
    get: (key: string) => memoryStoreGet(state, key),
    increment: (key: string, windowMs: number) => memoryStoreIncrement(state, key, windowMs),
 ...
 */
const createSlidingWindowStoreState = (cleanupIntervalMs = 60000): SlidingWindowStoreState => {
 ...
 */
const slidingWindowStoreReset = (state: SlidingWindowStoreState, key: string): Promise<void> => {
 ...
 */
export const createSlidingWindowStore = (cleanupIntervalMs = 60000): RateLimitStore => {
  const state = createSlidingWindowStoreState(cleanupIntervalMs);

  return {
    get: (key: string) => slidingWindowStoreGet(state, key),
    increment: (key: string, windowMs: number) => slidingWindowStoreIncrement(state, key, windowMs),
...
const tokenBucketStoreGet = (
  ...
  return Promise.resolve({
    ...
    resetTime: Date.now() + ((state.bucketSize - bucket.tokens) / state.refillRate) * 1000,
...
const tokenBucketStoreIncrement = (
  ...
  return Promise.resolve({
    totalHits: state.bucketSize - bucket.tokens,
    resetTime: now + ((state.bucketSize - bucket.tokens) / state.refillRate) * 1000,
    remaining: bucket.tokens,
    windowStart: bucket.lastRefill,
  });
};
 ...
 */
const tokenBucketStoreReset = (state: TokenBucketStoreState, key: string): Promise<void> => {
...
export const createTokenBucketStore = (
  ...
  const state = createTokenBucketStoreState(bucketSize, refillRate, cleanupIntervalMs);

  return {
    get: (key: string) => tokenBucketStoreGet(state, key),
    increment: (key: string, windowMs: number) => tokenBucketStoreIncrement(state, key, windowMs),
  ...
  if (config.store) {
    ...
    switch (config.strategy) {
      case "sliding-window":
        ...
        break;
      case "token-bucket":
        store = createTokenBucketStore(config.max, config.max / (config.windowMs / 1000));
        break;
      default:
 ...
 */
export function rateLimitByIP(max = 100, windowMs = 15 * 60 * 1000): Middleware {
  return rateLimit({
    max,
    windowMs,
    keyGenerator: defaultKeyGenerator,
 ...
 */
export function rateLimitByEndpoint(max = 50, windowMs = 15 * 60 * 1000): Middleware {
  return rateLimit({
    max,
    windowMs,
    keyGenerator: (req) => {
 ...
 */
export function rateLimitByUser(max = 200, windowMs = 15 * 60 * 1000): Middleware {
  return rateLimit({
    max,
    windowMs,
    keyGenerator: (req) => {
...
export class TokenBucketStore {
  ...
  constructor(bucketSize = 10, refillRate = 1, cleanupIntervalMs = 60000) {
    this.store = createTokenBucketStore(bucketSize, refillRate, cleanupIntervalMs);
  }
