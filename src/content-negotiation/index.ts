...
export const matchesMediaType = (mediaType: string, pattern: string): boolean => {
  ...
  // Handle wildcards
  if (patternParsed.type === "*") {
    return true;
  }
  if (parsed.type !== patternParsed.type) {
    return false;
  }

  if (patternParsed.subtype === "*") {
    return true;
  }
  if (parsed.subtype !== patternParsed.subtype) {
    return false;
  }
...
export const negotiateCharset = (
  ...
  for (const acceptItem of accepted) {
    for (const availableCharset of available) {
      if (
        acceptItem.value === "*" ||
        acceptItem.value.toLowerCase() === availableCharset.toLowerCase()
      ) {
...
export const negotiateEncoding = (
  ...
  for (const acceptItem of accepted) {
    for (const availableEncoding of available) {
      if (
        acceptItem.value === "*" ||
        acceptItem.value.toLowerCase() === availableEncoding.toLowerCase()
      ) {
...
export const respondWithBestFormat = (
  req: VerbRequest,
  data: any,
  formats: Record<string, (data: any) => string>,
) => {
