...
export interface WebSocketsServerInstance extends ServerInstance {
  websocket: (handlers: WebSocketHandlers) => void;
  withTLS: (options: WebSocketsOptions) => WebSocketsServerInstance;
}

export const createWebSocketsServer = (): WebSocketsServerInstance => {
  ...
  let tlsOptions: WebSocketsOptions | null = null;
  ...
  const withTLS = (options: WebSocketsOptions): WebSocketsServerInstance => {
    tlsOptions = options;
    return wssServer;
  };
  ...
  const logRoutes = () => {
    console.log("\n📋 WebSocket Secure Server Routes:");
    console.log("===================================");
    ...
    // Show WebSocket info
    if (Object.keys(websocketHandlers).length > 0) {
      console.log("  WebSocket Handlers:");
      Object.keys(websocketHandlers).forEach((event) => {
        console.log(`    ${event.padEnd(7)} (WebSocket event)`);
      });
    }
  ...
  const listen = (port?: number, hostname?: string) => {
    if (!tlsOptions) {
      throw new Error("WebSocket Secure server requires TLS options. Use withTLS() to configure.");
    }
    ...
    const config: any = {
      ...
      // TLS configuration for secure WebSocket
      tls: tlsOptions,
  ...
  const wssServer: WebSocketsServerInstance = {
    get,
    post,
    put,
    delete: del,
