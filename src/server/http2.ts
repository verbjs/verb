...
const getResourceType = (mimeType: string): string => {
  ...
  if (mimeType.startsWith("application/javascript") || mimeType.startsWith("text/javascript")) {
    return "script";
  }
...
export const createHttp2Server = (options: Http2ServerOptions) => {
  ...
  console.log(`HTTP/2 server running at https://${hostname}:${port} (HTTP/2 enabled)`);
