...
export type HttpsServerInstance = ServerInstance & {
  withTLS: (options: HttpsOptions) => HttpsServerInstance;
};

export const createHttpsServer = (): HttpsServerInstance => {
  ...
  let tlsOptions: HttpsOptions | null = null;
  ...
  const withTLS = (options: HttpsOptions): HttpsServerInstance => {
    tlsOptions = options;
    return httpsServer;
  };
  ...
  const listen = (port?: number, hostname?: string) => {
    if (!tlsOptions) {
      throw new Error("HTTPS server requires TLS options. Use withTLS() to configure.");
    }
  ...
  const httpsServer: HttpsServerInstance = {
    get,
    post,
    put,
    delete: del,
